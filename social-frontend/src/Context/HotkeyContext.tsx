...
interface HotkeyContextValue {
    registerHotkey: (key: string, callback: Callback, scope: "global" | "local", region?: "left" | "center" | "right") => string;
...
export function HotkeyProvider({ children }: { children: React.ReactNode }) {
    ...
    const registerHotkey = (key: string, callback: Callback, scope: "global" | "local", region?: "left" | "center" | "right") => {
        if (!key) return "";
    ...
    useEffect(() => {
        const handler = (e: KeyboardEvent) => {
            ...
            for (const hotkey of hotkeys.current) {
                ...
                // LOCAL hotkeys must only fire when region is active
                if (hotkey.scope === "local") {
                    if (hotkey.region && focus.region !== hotkey.region) continue;
                }
...
export function useHotkeyManager() {
    ...
    if (!ctx) throw new Error("useHotkeyManager must be used inside <HotkeyProvider>");
    return ctx;
}
